---
layout: post
title: Installing Spinnaker on Kubernetes
order: 20
published: true
redirect_from:
  - /spinnaker_install_admin_guides/install_on_kubernetes/
  - /spinnaker_install_admin_guides/install-on-kubernetes/
  - /spinnaker-install-admin-guides/install_on_kubernetes/
---

This guide describes how to install Spinnaker in Kubernetes.

It uses an AWS S3 bucket for persistent storage of Spinnaker Applications and Pipelines; this can be swapped out for an equivalent blob storage mechanism such as Minio, GCS, or AZS.
...
* You have a Kubernetes cluster up and running, with the following:
  * Ability to access the Kubernetes API with `cluster-admin` permissions

* You have access to an AWS account, with the following:
...
* You have a persistent working directory in which to work in.  One option here is `~/spinnaker`
...
1. Create the local working directory on your workstation.  For the purposes of this document, we will be using `~/spinnaker`, but this can be any persistent directory on any Linux or OSX machine.
...
**You can use some other blob storage mechanism, such as Minio, using the instructions listed in the Open Source Spinnaker documentation: https://www.spinnaker.io/setup/install/storage/**
...
### If you want to use a specific folder/key in the bucket
...
**If you have another ingress controller, you can use that.  More importantly, you can use any ingress or service configuration that is available in your environment, as long as you can expose both the `spin-deck` and `spin-gate` endpoints to your end users.  Then, skip down to the section called _Configure Spinnaker to be aware of its endpoints_**

Follow the instructions in the NGINX Ingress Controller [documentation](https://kubernetes.github.io/ingress-nginx/deploy/) to install the NGINX Ingress Controller in your Kubernetes cluster, if you don't currently have an Ingress Controller.
...
Create a Kubernetes Ingress manifest to expose spin-deck and spin-gate (change your hosts and namespace accordingly, and if you're using a different Ingress controller, change the annotations, as well):
